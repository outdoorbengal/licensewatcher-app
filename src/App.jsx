...
function DashboardPage({ registrations, onNavigate, onAction }) {
  ...
  const alerts = useMemo(() => registrations.filter(r => (r.daysLeft < 10 || r.status === "Expired") && r.status !== "Approved" && r.status !== "Rejected").sort((a, b) => a.daysLeft - b.daysLeft).slice(0, 6), [registrations]);
  const expiring = useMemo(() => registrations.filter(r => r.daysLeft > 0 && r.daysLeft <= 90 && r.status !== "Approved" && r.status !== "Rejected").sort((a, b) => a.daysLeft - b.daysLeft).slice(0, 5), [registrations]);
...
function ActionModal({ reg, open, onClose, onUpdate, stateReqs, saving, db, products }) {
  ...
  const handleSave = () => { const ct = docs.filter(d => d.uploaded).length; onUpdate({ ...reg, documents: docs, uploadedDocs: ct, status, deadline: reg.deadline || "", notes, lastActionDate: new Date().toISOString().split("T")[0] }); };
