...
function RegistrationsPage({ registrations, stateReqs, onAction, onNewReg, onDelete, onEditProduct }) {
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
          ...
          <tbody>{filtered.map((r, i) => (
            <tr key={r.id} style={{ borderBottom: `1px solid ${C.borderLight}`, background: i % 2 === 0 ? C.surface : C.bg }}>
              <td style={{ padding: "12px 16px", fontWeight: 600 }}><span onClick={() => onEditProduct?.(r.productName)} style={{ cursor: "pointer", color: C.primary, textDecoration: "none" }} onMouseEnter={e => e.currentTarget.style.textDecoration = "underline"} onMouseLeave={e => e.currentTarget.style.textDecoration = "none"}>{r.productName}</span></td>
...
function ActionModal({ reg, open, onClose, onUpdate, stateReqs, saving, db, products }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Registration Action" width={680}>
      ...
      {/* State Contact */}
      {st && (st.contact || st.email || st.phone) && (<div style={{ padding: 16, borderRadius: 10, border: `1px solid ${C.borderLight}`, marginBottom: 14 }}>
      ...
      {/* Submission Info */}
      {st && (st.onlinePortalUrl || st.mailingAddress || st.checkPayee) && (<div style={{ padding: 16, borderRadius: 10, border: `1px solid ${C.primaryLight}`, background: C.primaryFaint, marginBottom: 14 }}>
      ...
      {st && (st.registrationFormFile || st.registrationFormUrl) && (<div style={{ padding: 16, borderRadius: 10, border: `1px solid ${C.borderLight}`, marginBottom: 14 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}><I n="file" s={16} c={C.primary} /><span style={{ fontWeight: 700, fontSize: 14 }}>State Registration Form</span></div>
        {st.registrationFormFile ? (
          <FileDisplay label="Blank registration form" filePath={st.registrationFormFile} db={db} />
        ) : (
          <a href={st.registrationFormUrl} target="_blank" rel="noopener noreferrer" style={{ ...S.btn("outline"), padding: "6px 12px", fontSize: 13, textDecoration: "none" }}><I n="download" s={14} c={C.primary} /> Download form from state website</a>
        )}
      </div>)}
      ...
      {/* Product Files */}
      {product && (product.labelFilePath || product.gaFilePath) && (<div style={{ padding: 16, borderRadius: 10, border: `1px solid ${C.borderLight}`, marginBottom: 14 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}><I n="cat" s={16} c={C.teal} /><span style={{ fontWeight: 700, fontSize: 14 }}>Product Documents</span></div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}>
        ...
        <Field label="Manual Deadline" note="Override or add custom date"><input type="date" value={deadline} onChange={e => setDeadline(e.target.value)} style={S.input} /></Field>
      </div>

      {/* Required Documents with Upload/Download/Print */}
      <div style={{ marginBottom: 14 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}><I n="file" s={16} c={C.purple} /><span style={{ fontWeight: 700, fontSize: 14 }}>Required Documents for this Registration</span></div>
        {docs.map((doc, i) => <FileUploadBtn key={i} label={doc.name} fileName={doc.uploaded ? doc.file : null} filePath={doc.filePath} onUpload={(file) => handleUpload(i, file)} uploading={uploadingIdx === i} db={db} />)}
...
function NewRegModal({ open, onClose, onCreate, stateReqs, saving, db }) {
  ...
  const handleCreate = () => {
    if (!valid) return;
    onCreate({ name, type, description: desc, states: selStates, stateDeadlines, labelFile: labelFile || "", gaFile: gaFile || "", labelFilePath: labelPath, gaFilePath: gaPath });
    setName(""); setType("Pet Treat"); setDesc(""); setSelStates([]); setStateDeadlines({}); setLabelFile(null); setGaFile(null); setLabelPath(""); setGaPath("");
    onClose();
  };
...
function Sidebar({ page, onNavigate, registrations, syncing, products, onEditProduct }) {
  ...
  const nav = [{ id: "dashboard", label: "Dashboard", icon: "dashboard" }, { id: "registrations", label: "All Registrations", icon: "list" }, { id: "deadlines", label: "Deadlines", icon: "calendar" }, { id: "library", label: "State Library", icon: "library" }, { id: "settings", label: "Settings", icon: "settings" }];
  return (
...
function ProductEditModal({ productName, open, onClose, products, registrations, onSaveProduct, saving, db }) {
  ...
  const handleSave = () => {
    onSaveProduct({ ...product, description: desc, labelFile, labelFilePath: labelPath, gaFile, gaFilePath: gaPath });
  };
  ...
  return (
    <Modal open={open} onClose={onClose} title={`Edit: ${productName}`} width={640}>
      <div style={{ marginBottom: 16, fontSize: 14, color: C.textSec }}>
        Type: <strong>{product.type}</strong> • Registered in <strong>{productRegs.length}</strong> state{productRegs.length === 1 ? "" : "s"}
      </div>
...
// ═══════════════════════════════════════════════════════════════════════════════
// DEADLINES VIEW — chronological pipeline of all upcoming deadlines
// ═══════════════════════════════════════════════════════════════════════════════

function DeadlinesPage({ registrations, onAction }) {
  const [horizon, setHorizon] = useState(90); // days

  // Expand registrations into individual deadline instances
  const allDeadlines = useMemo(() => {
    const items = [];
    registrations.forEach(r => {
      (r.upcomingDeadlines || []).forEach(d => {
        if (d.daysLeft <= horizon) items.push({ ...d, reg: r });
      });
    });
    items.sort((a, b) => a.daysLeft - b.daysLeft);
    return items;
  }, [registrations, horizon]);
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ margin: 0, fontSize: 28, fontWeight: 800 }}>Upcoming Deadlines</h1>
        <p style={{ margin: "4px 0 0", color: C.textSec, fontSize: 15 }}>Every filing, renewal, and tonnage report ahead of you</p>
      </div>

      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        {[{ label: "Next 3 months", v: 90 }, { label: "Next 6 months", v: 180 }, { label: "Next 12 months", v: 365 }].map(opt => (
          <button key={opt.v} onClick={() => setHorizon(opt.v)} style={{ ...S.btn(horizon === opt.v ? "primary" : "outline"), padding: "8px 14px", fontSize: 13 }}>{opt.label}</button>
        ))}
        <div style={{ marginLeft: "auto", fontSize: 13, color: C.textSec, alignSelf: "center" }}>{allDeadlines.length} deadline{allDeadlines.length === 1 ? "" : "s"}</div>
      </div>

      {grouped.length === 0 && (
        <div style={{ ...S.card, textAlign: "center", padding: 40, color: C.textTri }}>
          No deadlines in the selected time frame.
        </div>
      )}
      ...
      {grouped.map(([key, { label, items }]) => (
        <div key={key} style={{ marginBottom: 24 }}>
          ...
          <div style={{ ...S.card, padding: 0, overflow: "hidden" }}>
            {items.map((d, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 18px", borderBottom: i < items.length - 1 ? `1px solid ${C.borderLight}` : "none" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 13, color: C.textSec }}>{d.reg.productName} <span style={{ color: C.textTri }}>•</span> {d.reg.stateName}</div>
                </div>
                ...
                <button onClick={() => onAction(d.reg)} style={{ ...S.btn("outline"), padding: "6px 14px", fontSize: 12 }}>Action</button>
              </div>
            ))}
          </div>
        </div>
...
export default function App() {
  ...
  const handleCreate = async ({ name, type, description, states, stateDeadlines, labelFile, gaFile, labelFilePath, gaFilePath }) => {
    setSyncing(true);
    const now = new Date();
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: C.bg, fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, sans-serif", color: C.text }}>
      ...
      <div style={{ flex: 1, padding: "28px 36px", maxWidth: 1200, overflowX: "auto" }}>
        {loading ? (<div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: 400, gap: 16 }}><Spinner size={32} /><div style={{ fontSize: 15, color: C.textSec }}>Loading from GitHub...</div></div>) : (<>
          ...
          {page === "registrations" && <RegistrationsPage registrations={registrations} stateReqs={stateReqs} onAction={setActionReg} onNewReg={() => setShowNewReg(true)} onDelete={handleDelete} onEditProduct={setEditProductName} />}
          {page === "deadlines" && <DeadlinesPage registrations={registrations} onAction={setActionReg} />}
      ...
      <ActionModal reg={actionReg} open={!!actionReg} onClose={() => setActionReg(null)} onUpdate={handleUpdate} stateReqs={stateReqs} saving={syncing} db={db} products={products} />
