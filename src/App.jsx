              ...
              <div style={{ padding: 16, borderRadius: 10, border: `2px solid ${currentDlStatus === "Approved" ? C.green : currentDlStatus === "Submitted" ? C.primary : C.border}`, background: currentDlStatus === "Approved" ? C.greenLight : currentDlStatus === "Submitted" ? C.primaryFaint : C.bg }}>
                ...
                <div style={{ display: "flex", gap: 10, marginBottom: currentDlStatus === "Approved" ? 14 : 0 }}>
                  <button onClick={() => { setDeadlineStatus("Submitted"); setShowApprovalPrompt(false); }} style={{
                    ...
                    background: currentDlStatus === "Submitted" ? C.primary : currentDlStatus === "Approved" ? C.primaryLight : "transparent",
                    color: currentDlStatus === "Submitted" ? "#fff" : C.primary,
                    borderColor: C.primary,
                  }}>
                    {(currentDlStatus === "Submitted" || currentDlStatus === "Approved") && <span>✓</span>} Submitted
                  </button>
                  <button onClick={() => { setDeadlineStatus("Approved"); setShowApprovalPrompt(true); }} style={{
                    ...S.btn(currentDlStatus === "Approved" ? "primary" : "outline"),
                    flex: 1, justifyContent: "center", padding: "12px 16px", fontSize: 14,
                    background: currentDlStatus === "Approved" ? C.green : "transparent",
                    color: currentDlStatus === "Approved" ? "#fff" : C.green,
                    borderColor: C.green,
                  }}>
...
export default function App() {
  ...
  const handleSaveState = async (code, data) => {
    ...
    try {
      ...
      setStateReqs(updated);
      // Re-enrich registrations with new state deadlines
      setRegistrations(enrichRegs(registrations.map(({ daysLeft, priority, upcomingDeadlines, nearestDeadlineLabel, ...r }) => r), updated));
  ...
  const handleBulkSave = async (allStates) => {
    if (!db) return; setSyncing(true);
    try {
      await db.write("data/state-offices.json", allStates, "Bulk update state offices");
      setStateReqs(allStates);
      // Re-enrich registrations with new state deadlines
      setRegistrations(enrichRegs(registrations.map(({ daysLeft, priority, upcomingDeadlines, nearestDeadlineLabel, ...r }) => r), allStates));
      showToast("All states updated");
