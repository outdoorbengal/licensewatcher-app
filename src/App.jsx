...
function SettingsPage({ settings, onSave, products, config, saving, activityLog, onClearLog }) {
  ...
  const copyLog = () => {
    const text = activityLog.map(e => `[${e.time}] [${e.level.toUpperCase()}] ${e.action}: ${e.detail}`).join("\n");
      ...
      <div style={{ maxWidth: 600 }}>
        ...
        <div style={{ ...S.card, marginTop: 20 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={copyLog} style={{ ...S.btn("outline"), padding: "5px 12px", fontSize: 12 }}>Copy All</button>
              <button onClick={onClearLog} style={{ ...S.btn("outline"), padding: "5px 12px", fontSize: 12 }}>Clear</button>
            </div>
          </div>
          <div style={{ maxHeight: 300, overflow: "auto", fontFamily: "monospace", fontSize: 11, lineHeight: 1.6, background: C.bg, borderRadius: 8, padding: 10 }}>
            {activityLog.length === 0 && <div style={{ color: C.textTri, textAlign: "center", padding: 16 }}>No activity yet.</div>}
            {activityLog.map((e, i) => (
              <div key={i} style={{ padding: "3px 6px", borderRadius: 4, marginBottom: 1, background: e.level === "error" ? C.redLight : "transparent", color: e.level === "error" ? C.red : C.textSec }}>
...
function ActionModal({ reg, deadlineCtx, open, onClose, onUpdate, stateReqs, saving, db, products }) {
  ...
  useEffect(() => {
    if (reg) {
      ...
      // Initialize deadline status from existing approvals
      if (deadlineCtx) {
  ...
  const handleSave = () => {
    ...
    // If acting on a specific deadline, record its status
    if (deadlineCtx && deadlineStatus && deadlineStatus !== "Not Started") {
      approvals = {
        ...approvals,
  ...
  return (
    <Modal open={open} onClose={onClose} title="Registration Action" width={680}>
      ...
      {/* Status — different UI depending on whether acting on specific deadline or general */}
      {deadlineCtx ? (
        <div style={{ marginBottom: 18 }}>
          {(() => {
            const existing = (reg.approvals || {})[deadlineCtx.instanceKey];
            const dlStatus = existing?.status || "Not Started";
            const dlSteps = ["Not Started", "Submitted", "Approved"];
            const dlCurrent = dlSteps.indexOf(dlStatus);
            const setDlStatus = (s) => {
              setShowApprovalPrompt(s === "Approved");
              // Store the selected deadline status temporarily
              setDeadlineStatus(s);
            };
            return (
              <div style={{ padding: 16, borderRadius: 10, border: `2px solid ${showApprovalPrompt ? C.green : C.border}`, background: showApprovalPrompt ? C.greenLight : C.bg }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: C.textTri, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 8 }}>Deadline Status</div>
                <div style={{ display: "flex", alignItems: "center", gap: 0, background: C.surface, borderRadius: 10, padding: 4, border: `1px solid ${C.borderLight}`, marginBottom: showApprovalPrompt ? 14 : 0 }}>
                  {dlSteps.map((s, i) => {
                    const isActive = i === (deadlineStatus ? dlSteps.indexOf(deadlineStatus) : dlCurrent);
                    const isPast = i < (deadlineStatus ? dlSteps.indexOf(deadlineStatus) : dlCurrent);
                    const isReached = i <= (deadlineStatus ? dlSteps.indexOf(deadlineStatus) : dlCurrent);
                    return (
                      <div key={s} onClick={() => setDlStatus(s)} style={{
                        flex: 1, padding: "8px 6px", textAlign: "center", cursor: "pointer", borderRadius: 8,
                        background: isActive ? (s === "Approved" ? C.green : C.primary) : "transparent",
                        color: isActive ? "#fff" : isReached ? C.primary : C.textTri,
                        fontWeight: isActive ? 700 : isReached ? 600 : 500, fontSize: 11, transition: "all 0.2s",
            ...
            {steps.map((s, i) => {
              ...
              return (
                <div key={s} onClick={() => setStatus(s)} style={{
                  ...
                  fontWeight: active ? 700 : reached ? 600 : 500, fontSize: 13, transition: "all 0.2s",
      ...
      {/* Other Upcoming Deadlines (exclude the one we're acting on) */}
      {(() => {
        const otherDeadlines = (reg.upcomingDeadlines || []).filter(d => !d.approved && (!deadlineCtx || d.instanceKey !== deadlineCtx.instanceKey));
        return otherDeadlines.length > 0 ? (
          <div style={{ padding: 16, borderRadius: 10, border: `1px solid ${C.orangeBorder}`, background: C.orangeLight, marginBottom: 14 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}><I n="calendar" s={16} c={C.orange} /><span style={{ fontWeight: 700, fontSize: 14 }}>Other Upcoming Deadlines</span></div>
        ...
        for (let yearOffset = -1; yearOffset <= 2; yearOffset++) {
          ...
          if (date >= today && date <= horizonEnd) {
            ...
            upcomingDeadlines.push({
              ...
              dlStatus: approval?.status || "Not Started",
