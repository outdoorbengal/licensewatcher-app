...
function LibraryPage({ stateReqs, onSaveState, onBulkSave, saving }) {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const colMap = {};
      hdrs.forEach((h, i) => {
        if (h.includes("state code") || (h.includes("state") && h.includes("code"))) colMap.code = i;
        else if (h.includes("contact")) colMap.contact = i;
        else if (h.includes("email")) colMap.email = i;
        else if (h.includes("phone")) colMap.phone = i;
        else if (h.includes("fee") && !h.includes("tonnage")) colMap.fees = i;
        else if (h.includes("processing")) colMap.processingTime = i;
        else if (h.includes("tonnage") && h.includes("dead")) colMap.tonnageDeadline = i;
        else if (h.includes("renewal") && h.includes("dead")) colMap.renewalDeadline = i;
        else if (h.includes("renewal") && h.includes("period")) colMap.renewalPeriod = i;
        else if (h.includes("tonnage") && h.includes("freq")) colMap.tonnageFrequency = i;
        else if (h.includes("required") || h.includes("docs")) colMap.requirements = i;
        else if (h.includes("form") && h.includes("url")) colMap.registrationFormUrl = i;
        else if (h.includes("registration") && h.includes("url")) colMap.registrationFormUrl = i;
        else if (h.includes("note")) colMap.notes = i;
      });

      // Fallback to positional if no headers matched
      if (colMap.code == null) colMap.code = 0;

      const updated = { ...stateReqs };
      let loadedCount = 0;
      for (let i = headerIdx + 1; i < rows.length; i++) {
        const r = rows[i];
        if (!r || !r[colMap.code]) continue;
        const code = String(r[colMap.code]).trim().toUpperCase();
        ...
        // Convert old tonnage/renewal deadline text to deadlines array
        const existingDeadlines = updated[code].deadlines || [];
        const newDeadlines = [...existingDeadlines];
        const tonnageDl = val("tonnageDeadline");
        const renewalDl = val("renewalDeadline");
        // Only add if we have values and don't already have these deadlines
        if (tonnageDl && !newDeadlines.find(d => d.title === "Tonnage Report")) {
          const parsed = parseDateText(tonnageDl);
          if (parsed) newDeadlines.push({ title: "Tonnage Report", ...parsed });
        }
        if (renewalDl && !newDeadlines.find(d => d.title === "Renewal")) {
          const parsed = parseDateText(renewalDl);
          if (parsed) newDeadlines.push({ title: "Renewal", ...parsed });
        }

        updated[code] = {
          ...updated[code],
          contact: val("contact") || updated[code].contact || "",
          email: val("email") || updated[code].email || "",
          phone: val("phone") || updated[code].phone || "",
          fees: val("fees") || updated[code].fees || "",
          processingTime: val("processingTime") || updated[code].processingTime || "",
          renewalPeriod: val("renewalPeriod") || updated[code].renewalPeriod || "",
          tonnageFrequency: val("tonnageFrequency") || updated[code].tonnageFrequency || "",
          requirements: val("requirements") ? val("requirements").split(";").map(s => s.trim()).filter(Boolean) : updated[code].requirements || [],
          registrationFormUrl: val("registrationFormUrl") || updated[code].registrationFormUrl || "",
          notes: val("notes") || updated[code].notes || "",
          deadlines: newDeadlines.slice(0, 5),
        };
        loadedCount++;
      }
      onBulkSave(updated);
      alert(`Loaded data for ${loadedCount} states.`);
    } catch (err) { console.error("XLSX parse error:", err); alert("Error parsing file: " + err.message + "\nPlease use the .xlsx template from Download Template."); }
    e.target.value = "";
  };
