...
const BLANK = { contact: "", email: "", phone: "", processingTime: "", fees: "", requirements: [], notes: "", registrationFormUrl: "", tonnageDeadline: "", renewalDeadline: "" };
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
          ...
          <tbody>{filtered.map((r, i) => (
            <tr key={r.id} style={{ borderBottom: `1px solid ${C.borderLight}`, background: i % 2 === 0 ? C.surface : C.bg }}>
              ...
              <td style={{ padding: "12px 16px" }}><div style={{ fontWeight: 500 }}>{r.deadline || "—"}</div>{r.deadline && <div style={{ fontSize: 11, fontWeight: 600, color: r.daysLeft < 0 ? C.red : r.daysLeft < 10 ? C.orange : C.textTri }}>{r.daysLeft < 0 ? `${Math.abs(r.daysLeft)}d overdue` : `${r.daysLeft}d left`}</div>}</td>
...
function LibraryPage({ stateReqs, onSaveState, onBulkSave, saving }) {
  ...
  const downloadTemplate = () => {
    const headers = ["State Code","State Name","Contact Name","Email","Phone","Processing Time","Fees","Tonnage Deadline","Renewal Deadline","Required Documents (semicolon-separated)","Registration Form URL","Notes"];
    const rows = Object.entries(stateReqs).map(([code, st]) => [
      code, st.name, st.contact || "", st.email || "", st.phone || "", st.processingTime || "", st.fees || "",
      st.tonnageDeadline || "", st.renewalDeadline || "", (st.requirements || []).join(";"), st.registrationFormUrl || "", st.notes || ""
    ]);
    const csv = [headers, ...rows].map(r => r.map(c => `"${String(c).replace(/"/g, '""')}"`).join(",")).join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = "state_offices_template.csv"; a.click();
      ...
      try {
        ...
        const lines = text.split("\n").map(l => {
          ...
          for (let i = 0; i < l.length; i++) {
            const ch = l[i];
            if (ch === '"') { if (inQuote && l[i + 1] === '"') { cur += '"'; i++; } else { inQuote = !inQuote; } }
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          const code = r[0].replace(/"/g, "").trim().toUpperCase();
          if (!updated[code]) continue;
          updated[code] = {
            ...updated[code],
            contact: r[2] || updated[code].contact || "",
            email: r[3] || updated[code].email || "",
            phone: r[4] || updated[code].phone || "",
            processingTime: r[5] || updated[code].processingTime || "",
            fees: r[6] || updated[code].fees || "",
            tonnageDeadline: r[7] || updated[code].tonnageDeadline || "",
            renewalDeadline: r[8] || updated[code].renewalDeadline || "",
            requirements: r[9] ? r[9].split(";").map(s => s.trim()).filter(Boolean) : updated[code].requirements || [],
            registrationFormUrl: r[10] || updated[code].registrationFormUrl || "",
            notes: r[11] || updated[code].notes || "",
          };
        }
        onBulkSave(updated);
    ...
    return (
      <div style={{ ...S.card, cursor: "pointer", transition: "box-shadow 0.15s, transform 0.15s", position: "relative" }} onClick={() => startEdit(code)} onMouseEnter={e => { e.currentTarget.style.boxShadow = "0 4px 20px rgba(0,0,0,0.07)"; e.currentTarget.style.transform = "translateY(-2px)"; }} onMouseLeave={e => { e.currentTarget.style.boxShadow = "none"; e.currentTarget.style.transform = "none"; }}>
        ...
        {hasData && (<>
          ...
          {(st.tonnageDeadline || st.renewalDeadline) && <div style={{ display: "flex", gap: 20, marginBottom: 10 }}>
            {st.tonnageDeadline && <div><div style={{ fontSize: 11, color: C.textTri, fontWeight: 600 }}>Tonnage Deadline</div><div style={{ fontSize: 13, fontWeight: 600 }}>{st.tonnageDeadline}</div></div>}
            {st.renewalDeadline && <div><div style={{ fontSize: 11, color: C.textTri, fontWeight: 600 }}>Renewal Deadline</div><div style={{ fontSize: 13, fontWeight: 600 }}>{st.renewalDeadline}</div></div>}
          </div>}
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
          <button onClick={downloadTemplate} style={S.btn("outline")}><I n="download" s={16} c={C.textSec} /> Download CSV</button>
          <label style={{ ...S.btn("primary"), cursor: "pointer" }}><I n="upload" s={16} c="#fff" /> Upload CSV<input type="file" accept=".csv" onChange={handleCsvUpload} style={{ display: "none" }} /></label>
        </div>
      </div>
      ...
      <Modal open={!!editing} onClose={() => setEditing(null)} title={editing ? `Edit ${editing} — ${form.name}` : ""} width={640}>
        {editing && (<div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <Field label="Tonnage Report Deadline"><input type="date" value={form.tonnageDeadline || ""} onChange={e => updateForm("tonnageDeadline", e.target.value)} style={S.input} /></Field>
            <Field label="Renewal Deadline"><input type="date" value={form.renewalDeadline || ""} onChange={e => updateForm("renewalDeadline", e.target.value)} style={S.input} /></Field>
          </div>
...
export default function App() {
  ...
  const enrichRegs = useCallback((regs) => {
    const now = new Date();
    return regs.map(r => {
      if (!r.deadline) return { ...r, daysLeft: 999, priority: "Low" };
  ...
  const loadData = useCallback(async (database) => {
    ...
    try {
      ...
      setProducts(prods || []);
      setStateReqs(sreqs || DEFAULT_STATES);
  ...
  const saveRegs = useCallback(async (newRegs) => {
    ...
    try {
      ...
      setRegistrations(enrichRegs(newRegs)); showToast("Saved to GitHub");
  ...
  const handleUpdate = async (updated) => {
    ...
    const newRegs = registrations.map(r => r.id === updated.id ? clean : (() => { const { daysLeft: dl, priority: pr, ...rest } = r; return rest; })());
