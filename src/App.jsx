...
function DashboardPage({ registrations, onNavigate, onAction }) {
  ...
  const alerts = useMemo(() => allDeadlines.filter(d => d.daysLeft < 10).slice(0, 6), [allDeadlines]);
  const upcomingTop = useMemo(() => allDeadlines.slice(0, 10), [allDeadlines]);
      ...
      {alerts.length > 0 && (
        <div style={{ ...S.card, marginBottom: 18 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {alerts.map((d) => (
              <div key={`${d.reg.id}_${d.instanceKey}`} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", borderRadius: 10, background: d.daysLeft < 0 ? C.redLight : C.orangeLight, border: `1px solid ${d.daysLeft < 0 ? C.redBorder : C.orangeBorder}` }}>
                <I n="cat" s={18} c={d.daysLeft < 0 ? C.red : C.orange} />
                ...
                <span style={{ fontSize: 12, fontWeight: 700, color: d.daysLeft < 0 ? C.red : C.orange, background: d.daysLeft < 0 ? C.redLight : C.orangeLight, padding: "4px 10px", borderRadius: 10, border: `1px solid ${d.daysLeft < 0 ? C.redBorder : C.orangeBorder}` }}>{d.daysLeft < 0 ? `${Math.abs(d.daysLeft)}d overdue` : `${d.daysLeft}d left`}</span>
...
function DeadlinesPage({ registrations, stateReqs, onAction, onNewReg, onEditProduct }) {
    ...
    <div>
      ...
      {grouped.length === 0 && (
        <div style={{ ...S.card, textAlign: "center", padding: 40, color: C.textTri }}>
          {registrations.length === 0 ? "No products registered yet. Click 'Register New Product' to begin." : showArchived ? "No archived deadlines." : "No deadlines match these filters."}
        </div>
      )}
...
function ProductsPage({ registrations, stateReqs, products, onEditProduct, onBulkUpdate, onUpdateRegStatus, saving }) {
...
function ActionModal({ reg, deadlineCtx, open, onClose, onUpdate, stateReqs, saving, db, products }) {
  ...
  const handleSave = () => {
    ...
    // If approving a specific deadline instance and file uploaded, record it
    if (status === "Approved" && deadlineCtx && approvalFilePath) {
      approvals = {
        ...approvals,
    ...
    onUpdate({ ...reg, documents: docs, uploadedDocs: ct, status, deadline: reg.deadline || "", notes, approvals, lastActionDate: new Date().toISOString().split("T")[0] });
  };
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: C.bg, fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, sans-serif", color: C.text }}>
      ...
      <div style={{ flex: 1, padding: "28px 36px", maxWidth: 1200, overflowX: "auto" }}>
        {loading ? (<div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: 400, gap: 16 }}><Spinner size={32} /><div style={{ fontSize: 15, color: C.textSec }}>Loading from GitHub...</div></div>) : (<>
          ...
          {page === "deadlines" && <DeadlinesPage registrations={registrations} stateReqs={stateReqs} onAction={handleAction} onNewReg={() => setShowNewReg(true)} onEditProduct={setEditProductName} />}
          {page === "registrations" && <ProductsPage registrations={registrations} stateReqs={stateReqs} products={products} onEditProduct={setEditProductName} onBulkUpdate={handleBulkUpdate} onUpdateRegStatus={handleUpdateRegStatus} saving={syncing} />}
