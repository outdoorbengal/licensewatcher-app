...
function LibraryPage({ stateReqs, onSaveState, saving }) {
  const [search, setSearch] = useState("");
  const [editing, setEditing] = useState(null); // state code being edited
    ...
    return (
      <div style={{ ...S.card, cursor: "pointer", transition: "box-shadow 0.15s, transform 0.15s", position: "relative" }} onClick={() => startEdit(code)} onMouseEnter={e => { e.currentTarget.style.boxShadow = "0 4px 20px rgba(0,0,0,0.07)"; e.currentTarget.style.transform = "translateY(-2px)"; }} onMouseLeave={e => { e.currentTarget.style.boxShadow = "none"; e.currentTarget.style.transform = "none"; }}>
        ...
        {hasData && (<>
          ...
          {hasForms && <div style={{ marginTop: 8, fontSize: 12, color: C.primary, display: "flex", alignItems: "center", gap: 4 }}><I n="file" s={12} c={C.primary} /> Registration form on file</div>}
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}><h1 style={{ margin: 0, fontSize: 28, fontWeight: 800 }}>State Offices Library</h1><p style={{ margin: "4px 0 0", color: C.textSec, fontSize: 15 }}>Contact info, requirements, deadlines, and forms for each state — click any card to edit</p></div>
      ...
      <Modal open={!!editing} onClose={() => setEditing(null)} title={editing ? `Edit ${editing} — ${form.name}` : ""} width={640}>
        {editing && (<div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <Field label="Tonnage Report Deadline" note="e.g. January 31, July 1"><input value={form.tonnageDeadline || ""} onChange={e => updateForm("tonnageDeadline", e.target.value)} placeholder="January 31" style={S.input} /></Field>
            <Field label="Renewal Deadline" note="e.g. Annually by Dec 31"><input value={form.renewalDeadline || ""} onChange={e => updateForm("renewalDeadline", e.target.value)} placeholder="December 31" style={S.input} /></Field>
          </div>
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: C.bg, fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, sans-serif", color: C.text }}>
      ...
      <div style={{ flex: 1, padding: "28px 36px", maxWidth: 1200, overflowX: "auto" }}>
        {loading ? (<div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: 400, gap: 16 }}><Spinner size={32} /><div style={{ fontSize: 15, color: C.textSec }}>Loading from GitHub...</div></div>) : (<>
          ...
          {page === "library" && <LibraryPage stateReqs={stateReqs} onSaveState={handleSaveState} saving={syncing} />}
