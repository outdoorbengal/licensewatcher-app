...
function ActionModal({ reg, deadlineCtx, open, onClose, onUpdate, stateReqs, saving, db, products }) {
  ...
  const handleSave = () => {
    ...
    // If the approval prompt was shown (user clicked Approved on a deadline), record it
    if (showApprovalPrompt && deadlineCtx) {
      ...
      // Keep the overall registration status unchanged — only the deadline instance is approved
    ...
    <Modal open={open} onClose={onClose} title="Registration Action" width={680}>
      ...
      {deadlineCtx ? (
        <div style={{ marginBottom: 18 }}>
          <div style={{ padding: 16, borderRadius: 10, border: `2px solid ${showApprovalPrompt ? C.green : C.border}`, background: showApprovalPrompt ? C.greenLight : C.bg, marginBottom: 10 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: showApprovalPrompt ? 12 : 0 }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: 700, color: C.textTri, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 4 }}>Deadline Action</div>
                <div style={{ fontSize: 15, fontWeight: 600 }}>{deadlineCtx.title} — {deadlineCtx.year}</div>
              </div>
              {!showApprovalPrompt ? (
                <button onClick={() => setShowApprovalPrompt(true)} style={{ ...S.btn("primary"), padding: "10px 20px", background: C.green }}>
                  <I n="check" s={16} c="#fff" /> Mark as Done
                </button>
              ) : (
                <span style={{ ...S.badge(C.greenLight, C.green, C.greenBorder), fontSize: 13 }}>✓ Approving</span>
              )}
            </div>
            {showApprovalPrompt && (
              <div style={{ marginTop: 12 }}>
                ...
                <button onClick={() => setShowApprovalPrompt(false)} style={{ ...S.btn("outline"), padding: "5px 12px", fontSize: 12, marginTop: 6 }}>Cancel</button>
              </div>
            )}
          </div>
        </div>
